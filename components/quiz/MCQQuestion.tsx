                        ...
                        return (
                            <Fragment key={index}>
                                <button onClick={() => onOptionChange(option.text)} className={optionClass} disabled={isSubmitted || isReviewMode}>
                                    ...
                                    <span className="flex-1 text-left"><FormattedText text={option.text} variant="answer" /></span>
                                </button>
                                {(isReviewMode || isSubmitted) && isSelected && !option.isCorrect && option.explanation && (
                                    <div className="pl-14 -mt-3 mb-2 text-red-600/90 animate-fadeIn">
                                        <FormattedText text={option.explanation} variant="explanation" />
                                    </div>
                                )}
                            </Fragment>
                        );
