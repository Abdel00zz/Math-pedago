...
// A simple, self-contained MD5 implementation.
...
export function calculateContentVersion(chapter: ChapterData): string {
    const dataToHash = {
        class: chapter.class_type,
        chapter: chapter.chapter_name,
        sessionDates: [...chapter.session_dates].sort(),
        videos: chapter.videos,
        quiz: chapter.quiz_questions,
        exercises: chapter.exercises,
    };

    const contentString = JSON.stringify(dataToHash, (key, value) => {
    ...
    const hash = md5(contentString).substring(0, 6);
    return `v1.1.0-${hash}`;
}
